...
// ── Inlined from tools/semver.js @ 179ae75 ───────────────────────────────
...
describe('comparePrerelease — issue #283 regression', () => {
  ...
  // BigInt comparison handles arbitrarily large integers exactly.
  test('large numeric identifiers compare correctly above 2^53 (issue #283)', () => {
    // These two values differ by 1 but are above Number.MAX_SAFE_INTEGER.
    // Integer subtraction would round both to 9007199254740992, yielding diff=0 (wrong).
    // BigInt comparison returns -1/1 exactly.
    assert.equal(Math.sign(comparePrerelease('9007199254740993', '9007199254740992')),  1);
    assert.equal(Math.sign(comparePrerelease('9007199254740992', '9007199254740993')), -1);
    assert.equal(Math.sign(comparePrerelease('9007199254740993', '9007199254740993')),  0);
  });

  test('alphanumeric identifiers are unaffected by BigInt path', () => {
    // BigInt() only called when both identifiers are all-digit strings.
    // Mixed identifiers must still use lexicographic/numeric rules correctly.
    assert.equal(Math.sign(comparePrerelease('alpha', 'beta')), -1);
    assert.equal(Math.sign(comparePrerelease('rc1',   'rc2')),  -1);
    assert.equal(Math.sign(comparePrerelease('1',     'alpha')), -1); // numeric < alphanumeric
  });
});
